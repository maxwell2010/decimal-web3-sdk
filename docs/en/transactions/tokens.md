...
Buy an exact raw token amount with a maximum DEL input.
...
GasCenter conversion with a supplied owner permit; distinct from token selling.
...
Sell tokens for an exact DEL output, bounded by maximum raw token input.
...
Legacy-only minimum supply update; disabled unless explicitly opted in.
