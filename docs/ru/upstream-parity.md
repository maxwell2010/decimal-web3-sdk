...
Safe multisig; burn DEL; exact-output покупки/продажи; GasCenter.convertToDEL; минимальная эмиссия; создание/удаление/метаданные валидаторов; complete/penalty стейка; NFT token-reserve, hold/reset/complete и permit; смешанная рассылка. Сначала нужно сверить существующие token/NFT ABI, затем добавлять недостающие методы.
